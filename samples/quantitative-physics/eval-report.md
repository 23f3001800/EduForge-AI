# Quality evaluation — Physics (quantitative)

**0.92 / 1.00 — exemplary**

`##################..`  profile `quantitative`, grade band `9-10`, package `22222222-2222-4222-8222-222222222222`
...
| Bloom distribution |  0.92 | 0.08 | deterministic |  |
| Coverage |  1.00 | 0.15 | deterministic |  |
| Sequencing and load |  1.00 | 0.08 | deterministic |  |
| Grounding |  0.75 | 0.15 | deterministic |  |
| Activity variety and runnability |  0.98 | 0.14 | deterministic |  |
| Differentiation |  0.97 | 0.08 | deterministic |  |
| Assessment integrity |  0.90 | 0.12 | deterministic |  |
...
- `profile_required_fields` 1.00 (weight 0.12) — quantitative requires concepts, formulae, examples
...
- `citation_integrity` 1.00 (weight 0.55) — 8 evidence span(s) checked verbatim against the source
- `claim_support` 0.44 (weight 0.45) — 8 claim(s) checked; 3 in the ambiguous band (0.25-0.6), scored 0.5 and left for the judge

**Activity variety and runnability** —  0.98

- `type_variety` 1.00 (weight 0.25) — 2 distinct type(s) across 2
- `profile_fit` 1.00 (weight 0.10) — types the quantitative profile weights
...
- `kind_mix` 0.70 (weight 0.10) — against the quantitative assessment mix
...
- **BLOOM_NARROW_SPREAD** `/assessments/items` — marks land on 2 Bloom level(s) (apply, understand); the quantitative profile expects at least 3
