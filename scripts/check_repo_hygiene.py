...
SECRET_PATTERNS = {
    ...
    # Kept even though the Groq, Gemini, and Anthropic providers themselves were
    # removed: this scans committed *files*, not the configured provider list,
    # and a leaked key is a leaked key regardless of whether this codebase still
    # calls that provider. An operator env can still hold one (e.g. from before
    # the provider was dropped), and a secret scanner's job is to catch shapes
    # that leak, not the ones we endorse.
