...
def _merge_bases(parts: list[KnowledgeBase]) -> dict[str, Any]:
    """Union of section-level extractions, deduped on normalised name.
    ...
    by_key: dict[str, dict[str, dict[str, Any]]] = {field: {} for field in merged}

    def key_for(field: str, item: dict[str, Any]) -> str:
        for candidate in ("concept_id", "term", "name", "statement", "objective_id", "context"):
    ...
    for part in parts:
        ...
        for field in merged:
            if field == "keywords":
                merged[field].extend(payload.get("keywords") or [])
                continue
            for item in payload.get(field) or []:
                key = key_for(field, item)
    ...
    for field in merged:
        if field == "keywords":
            merged[field] = sorted(dict.fromkeys(merged[field]))
        else:
            merged[field] = list(by_key[field].values())
    return merged
    ...
    async def run(self, ctx: StageContext, state: dict[str, Any]) -> dict[str, Any]:
        async with stage_span(ctx, self.name) as span:
            ...
            if total_tokens <= SINGLE_CALL_TOKEN_BUDGET or len(chunks) <= 4:
                await span.progress(0.2, message="extracting")
                result = await self._llm.parse(
                    stage=self.name,
                    output_model=KnowledgeBase,
                    system=system,
                    user_content=(
                        f"{document_block(_chunk_text(chunks))}\n\n"
                        f"{evidence_rules(chunk_ids)}\n\n"
                        "Extract the complete educational representation."
                    ),
                )
                payload = result.value.model_dump(mode="json")
                if result.degraded:
                    span.warn("knowledge extraction degraded")
            else:
                groups = _group_by_section(chunks)
                await span.progress(0.15, message=f"map-reduce over {len(groups)} sections")
                parts: list[KnowledgeBase] = []
                for index, group in enumerate(groups, start=1):
                    part = await self._llm.parse(
                        stage=self.name,
                        output_model=KnowledgeBase,
                        system=system,
                        user_content=(
                            f"{document_block(_chunk_text(group))}\n\n"
                            f"{evidence_rules([c['chunk_id'] for c in group])}\n\n"
                            "Extract the educational representation for THIS SECTION only. "
                            "Objectives and the concept graph are assembled separately, so "
                            "focus on concepts, definitions, formulae, examples, "
                            "applications, and misconceptions."
                        ),
                    )
                    parts.append(part.value)
                    await span.progress(
                        0.15 + 0.5 * index / len(groups),
                        message=f"section {index} of {len(groups)}",
                    )
                payload = _merge_bases(parts)
